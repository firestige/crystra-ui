# Legacy Dashboard capacity contract

This document preserves the existing ordinary-panel rendering and semantic classification contract. Ordinary Cards may be placed in a grid without acquiring Widget m×n dimensions or the Widget size catalog. For Widgets, use [Dashboard layout](dashboard-panel-layout.md) and [Widget monitoring](widget-monitoring.md): the outer-size formula includes inter-cell gaps. The table capacity below belongs to the retained panel implementation; it does not add Widget size choices.
