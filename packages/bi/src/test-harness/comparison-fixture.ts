  ...
  return deliveries.flatMap((delivery) =>
    roles.map((role, index) => {
      ...
      return {
        ...
        version: delivery.workflowVersion,
        role,
        calls,
        input: calls * (1100 + seed * 37 + index * 211),
        output: calls * (430 + seed * 13 + index * 71),
      };
    }),
  );
}
export function buildComparisonData(
  ...
  const scope = deliveries.filter((delivery) =>
    versions.includes(delivery.workflowVersion),
  );
  const samples = comparisonSamples(scope);
  const rows = selectedRoles.map((role) => ({
    name: role,
    values: scope.map((delivery) => {
      const sample = samples.find(
        (row) => row.deliveryId === delivery.deliveryId && row.role === role,
      );
      if (!sample) return 0;
      return metric === "mean"
        ? Math.round(sample.input / sample.calls)
        : sample[metric];
    }),
  }));
  return {
    ...
    unit: " tokens",
    ordered: false,
    dimensions: scope.map(
      (delivery) => `${delivery.deliveryId} · ${delivery.workflowVersion}`,
    ),
    rows,
    domain: [0, Math.max(1, ...rows.flatMap((row) => row.values))],
  };
}
