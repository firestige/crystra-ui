...
it("compares only selected Delivery, Role and version identities", () => {
  ...
  const selected = buildComparisonData(
    records.slice(1, 2),
    ["Engineer"],
    ["v2"],
    "output",
  ...
  const sample = comparisonSamples(records.slice(1, 2)).find(
    (row) => row.role === "Engineer",
  )!;
  expect(selected.rows[0].values).toEqual([sample.output]);
  expect(
    buildComparisonData(records.slice(1, 2), ["Engineer"], ["v2"], "mean")
      .rows[0].values,
  ).toEqual([Math.round(sample.input / sample.calls)]);
  expect(
    buildComparisonData([], ["Engineer"], ["v2"], "input").dimensions,
  ).toEqual([]);
});
