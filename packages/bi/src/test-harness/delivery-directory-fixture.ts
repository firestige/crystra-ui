...
export const deliveryDirectoryRecords: DeliverySearchRecord[] = Array.from(
  { length: 48 },
  (_, index) => ({
    ...
    startedAt: new Date(
      Date.parse("2026-09-09T14:08:00Z") - index * 4 * 3600_000,
    ).toISOString(),
  }),
);
...
export const deliveryDirectorySearchFields: import("../domain/delivery-search").DeliverySearchField[] =
  [
    ...
    },
    {
      key: "taskId",
      label: "Task ID",
      placeholder: "输入完整 Task ID",
      match: "exact",
    },
    {
      key: "workflowRef",
      label: "Workflow",
      placeholder: "名称或 name@version",
      match: "workflow",
    },
    {
      key: "deliveryId",
      label: "Delivery ID",
      placeholder: "输入完整 Delivery ID",
      match: "exact",
    },
  ];
