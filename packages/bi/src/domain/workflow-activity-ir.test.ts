...
it("projects the implementation test-first ladder and deterministic Goal loop", () => {
  ...
  expect(main.edges).toEqual(
    expect.arrayContaining([
      expect.objectContaining({ from: "commit", to: "select-goal" }),
    ]),
  );
  expect(ladder.edges).toEqual(
    expect.arrayContaining([
      expect.objectContaining({ from: "calibrate", to: "implement" }),
