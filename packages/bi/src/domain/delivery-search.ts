...
export interface DeliverySearchCondition {
  id: string;
...
export function searchDeliveries(
  ...
  fields: readonly DeliverySearchField[],
) {
...
export function filterDeliveryResults(
  results: readonly DeliverySearchRecord[],
  filters: DeliveryFilters,
) {
  return results.filter(
    (record) =>
      (!filters.taskId || record.taskId === filters.taskId) &&
      (!filters.workflowId || record.workflowId === filters.workflowId) &&
      (!filters.workflowVersion ||
        record.workflowVersion === filters.workflowVersion),
  );
}
export function selectedDelivery(
  ...
  return (
    records.find((record) => record.deliveryId === id) ?? records[0] ?? null
  );
}
