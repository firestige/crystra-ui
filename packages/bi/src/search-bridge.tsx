...
for (const kind of ["task", "workflow"]) {
  ...
  const action = control.querySelector<HTMLButtonElement>(
    `[data-section-id=${kind}-search-action]`,
  )!;
  const close = control.querySelector<HTMLButtonElement>(
  ...
  function SidebarAdapter() {
    ...
    return (
      <ExpandableSearchField
        ...
        expanded={open}
        onValueChange={(value) => forward(input, value)}
...
for (const selector of [
  "#browser-search",
  "[data-section-id=plan-dag-search] input",
]) {
  ...
  createRoot(seat).render(
    <SearchField
      label={input.getAttribute("aria-label") || "搜索节点"}
      hideLabel
      size="compact"
      id={input.id || undefined}
      data-section-id={input.dataset.sectionId}
      leading={
        dag ? label.querySelector("span")?.textContent : <Icon name="search" />
      }
      placeholder={input.placeholder}
