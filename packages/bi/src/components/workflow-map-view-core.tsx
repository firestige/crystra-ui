...
export function WorkflowMapViewCore({
  initialIR,
  resolveLayout,
  examples,
  exploration = false,
  ...
  headerContainer,
  onQuote,
  renderCrystallization,
  resourceWorkspaceForTitle,
  onIdentity,
  mode,
  onWorkflow,
  requestedNode,
}: {
  initialIR: WorkflowMapIR;
  resolveLayout: WorkflowLayoutResolver;
  examples?: readonly WorkflowMapIR[];
  exploration?: boolean;
  precomputedLayout?: boolean;
  headerContainer?: HTMLElement | null;
  onQuote?: (text: string, objectId: string | null) => void;
  renderCrystallization?: (quote: (text: string) => void) => React.ReactNode;
  resourceWorkspaceForTitle?: (title: string) => string | undefined;
  onIdentity: (title: string, description: string) => void;
  mode: string;
  onWorkflow?: (ir: WorkflowMapIR) => void;
  requestedNode?: { id: string; seq: number } | null;
}) {
  const samples = examples ?? [initialIR];
  const [ir, setIR] = useState<WorkflowMapIR>(initialIR),
  ...
  useEffect(() => {
    ...
    const changed = (event: Event) => {
      ...
      if (detail.workspace !== resourceWorkspaceForTitle?.(ir.title)) return;
      setMessages((old) => [
        ...old,
        {
          role: "system",
  ...
  }, [ir.title, exploration, resourceWorkspaceForTitle]);
  const introducedCrystallization = useRef(false);
  useEffect(() => {
    if (
      exploration &&
      mode === "crystallization" &&
      !introducedCrystallization.current
    ) {
  ...
  useEffect(() => {
    ...
    setBusy(true);
    setError("");
    resolveLayout(currentIR, expanded, direction, bounds)
      .then((next) => {
        if (cancelled) return;
        setLayout(next);
        setBusy(false);
  ...
  }, [currentIR, expanded, direction, bounds, resolveLayout]);
  useEffect(() => {
  ...
  return (
    ...
    >
                ...
                {readonly ? (
                  ...
                  <>
                    <MapIconButton
                      aria-label="保存草稿"
                      appearance="ghost"
                      disabled={!exploration || !!candidate}
                      onClick={() => {
                        try {
                    ...
                    <Button
                      disabled={!exploration || !!candidate || busy || !!error}
      ...
      {mode === "crystallization" && renderCrystallization?.(quote)}
      ...
      <div className="map-stage">
        <div
          ...
          ref={(node) => {
            if (node) node.inert = !outlineOpen;
          }}
        >
          <div>
            <WorkflowMapOutline
