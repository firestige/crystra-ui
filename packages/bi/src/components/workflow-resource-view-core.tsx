...
export function WorkflowResourceViewCore({
  workspace,
  identityKey,
  declaredCatalog,
  exploration = false,
  ...
  const catalog =
    declaredCatalog ??
    resourceCatalog(workspace?.files || [], workspace?.nodes, workspace?.edges);
  ...
  const discuss = () => {
    if (!exploration) {
      if (resource && onDiscuss)
        onDiscuss({ resourceId: resource.id, path: file.path });
      return;
    }
          ...
          {resourceGroups.map((group) => {
            ...
            return entries.length || !query.trim() ? (
              <ResourceGroup
                ...
                onAdd={
                  exploration ? () => manage({ kind: "add", group }) : undefined
                }
              >
        ...
        >
          ...
          >
            ...
            >
              ...
              >
                <FileAction
                  disabled={!exploration && (!onDiscuss || !resource)}
                  label="在对话中讨论此文件"
                  icon="message-plus"
                  onClick={discuss}
                />
          ...
          {tab === "refs" ? (
            RelationView && resource ? (
              <RelationView
                selection={{ resourceId: resource.id, path: resource.path }}
