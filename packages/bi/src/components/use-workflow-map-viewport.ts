...
export function useWorkflowMapViewport(
  ...
  useEffect(() => {
        ...
        (event) =>
          ...
          (!event.button &&
            !(event.target as Element).closest(
              '[role="button"],.map-disclosure',
            )),
      )
      .on("start", (event) => {
        setMoving(true);
        if (event.sourceEvent) exploreRef.current?.();
      })
      .on("zoom", (event) =>
        setCamera({
          x: event.transform.x,
    ...
    behavior.current = z;
    const selection = select(element).call(z).on("dblclick.zoom", null);
    return () => {
      selection.interrupt().on(".zoom", null);
      behavior.current = null;
    };
  }, [host]);
  const move = (next: MapCamera, animate = true) => {
    const element = host.current,
      z = behavior.current;
    if (!element || !z) return;
    const selection = select(element);
    selection.interrupt();
    const transform = zoomIdentity.translate(next.x, next.y).scale(next.scale);
    if (!animate || matchMedia("(prefers-reduced-motion: reduce)").matches)
      selection.call(z.transform, transform);
    else selection.transition().duration(560).call(z.transform, transform);
  };
