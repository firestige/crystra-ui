...
const viewAliases: Record<string, Page> = {
  analysis: "traces",
  audit: "traces",
  compare: "reports",
  dashboard: "dashboard",
...
export function AnalysisObservationStudy() {
  ...
  const [page, setPage] = useState<Page>(
    viewAliases[initial.get("view") ?? ""] ??
      (unresolved ? "traces" : "dashboard"),
  );
  ...
  useLayoutEffect(() => {
    ...
    document
      .querySelector(
        `[data-section-id="analysis-${page === "reports" ? "reports" : page}-action"]`,
      )
  ...
  const widgets = overviewSources(period, overviewFilters);
  const queries = createObservationQueryCatalog(period, overviewFilters);
  ...
  const rangeView = rangePanel
    ...
    : undefined;
  return (
    <section className="wb-host obs-host" data-section-id="main-surface-host">
      <header
      ...
      >
        ...
        <div data-header-slot="navigation">
          <Tabs
            ...
            items={(Object.keys(headings) as Page[]).map((value) => ({
              value,
