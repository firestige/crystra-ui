// @ts-nocheck -- The accepted v8 imperative implementation is preserved below.
/* eslint-disable */
...
export function mountV8Sidebar(root, callbacks) {
  ...
  const observers=[];const frames=new Set();const timers=new Set();
  const document=new Proxy(ownerDocument,{get(target,key){
    if(key==='querySelector')return selector=>root.matches(selector)?root:root.querySelector(selector);
    if(key==='querySelectorAll')return selector=>root.querySelectorAll(selector);
    if(key==='addEventListener')return (type,fn,options)=>target.addEventListener(type,fn,{...(typeof options==='boolean'?{capture:options}:options),signal:lifetime.signal});
    const value=Reflect.get(target,key,target);return typeof value==='function'?value.bind(target):value;
  }});
  const window=new Proxy(ownerWindow,{get(target,key){
    if(key==='addEventListener')return (type,fn,options)=>target.addEventListener(type,fn,{...(typeof options==='boolean'?{capture:options}:options),signal:lifetime.signal});
    const value=Reflect.get(target,key,target);return typeof value==='function'?value.bind(target):value;
  }});
  const matchMedia=query=>{const media=ownerWindow.matchMedia(query);const add=media.addEventListener.bind(media);media.addEventListener=(type,fn,options)=>add(type,fn,{...options,signal:lifetime.signal});return media;};
  const requestAnimationFrame=fn=>{const id=ownerWindow.requestAnimationFrame(fn);frames.add(id);return id;};
  const setTimeout=(fn,ms)=>{const id=ownerWindow.setTimeout(fn,ms);timers.add(id);return id;};
  const clearTimeout=id=>ownerWindow.clearTimeout(id);
  const MutationObserver=class extends ownerWindow.MutationObserver{constructor(fn){super(fn);observers.push(this);}};
  const section=name=>document.querySelector(`[data-section-id="${name}"]`);
  ...
  (() => {
    ...
    const parse=html=>{const template=ownerDocument.createElement('template');template.innerHTML=html;return template.content.firstElementChild;};
  const template=parse(taskRowTemplates[1]);
  const workflowTemplate=parse(workflowRowTemplate);
  const bindRows=(items,list,kind)=>{
    const old=new Map([...list.children].map(node=>[node.dataset.objectId,node]));
    for(const item of items){let row=old.get(item.id);old.delete(item.id);
      if(!row){row=(kind==='task'?template:workflowTemplate).cloneNode(true);row.removeAttribute('data-section-id');row.dataset.objectId=item.id;
        row.querySelectorAll('[data-section-id]').forEach(node=>node.removeAttribute('data-section-id'));
        row.addEventListener('click',event=>{event.preventDefault();callbacks.onNavigate(kind,row.dataset.objectId,row.dataset.revision||undefined);});
      }
      row.dataset.revision=item.revision??'';
      if(kind==='workflow'){row.href='#workflow';row.dataset.workflowRevision=item.revision??'';row.removeAttribute('data-source-task-id');}
      row.dataset.taskId=item.id;row.dataset.workflowDefinitionId=item.id;
      row.dataset.createdAt=item.createdAt??'';row.dataset.lastEventAt=item.lastActivityAt??'';row.dataset.taskActive=item.inactive===true?'false':'true';
      const title=row.querySelector('[data-type="item-title"]')??(kind==='workflow'?row.querySelector('span'):null);if(title){title.textContent=item.title;title.title=item.title;}
      const meta=row.querySelector('[data-type="meta"]');if(meta){meta.textContent=kind==='workflow'?(item.revision??''):'';meta.hidden=kind==='task';}
      // Unknown activity/status/time/attention must not inherit the template fixture.
      if(kind==='task')row.querySelectorAll('span:not([data-type="item-title"]):not([data-type="meta"])').forEach(node=>{node.hidden=true;node.removeAttribute('title');});
      const selected=callbacks.route===kind&&callbacks.selectedId===item.id;row.setAttribute('aria-current',String(selected));
      if(kind==='task')row.className=parse(taskRowTemplates[selected?0:1]).className;
      list.append(row);
    }
    for(const row of old.values())row.remove();
  };
  section('new-task-action').addEventListener('click',()=>callbacks.onNewTask());
  section('host-settings').addEventListener('click',()=>callbacks.onOpenSettings());
  for(const [id,page] of [['all-tasks-action','tasks'],['all-workflows-action','workflows'],['analysis-dashboard-action','analysis-overview'],['analysis-traces-action','analysis-traces'],['analysis-reports-action','analysis-reports']]){
    const link=section(id);link.href='#'+page;link.addEventListener('click',event=>{event.preventDefault();callbacks.onNavigate(page);});
  }
  const update=next=>{callbacks=next;bindRows(next.tasks,taskList,'task');bindRows(next.workflows,workflowList,'workflow');renderTaskList();renderWorkflowList();refreshAttention();
    for(const [id,page] of [['all-tasks-action','tasks'],['all-workflows-action','workflows'],['analysis-dashboard-action','analysis-overview'],['analysis-traces-action','analysis-traces'],['analysis-reports-action','analysis-reports']]){const node=section(id);if(next.route===page)node.setAttribute('aria-current','page');else node.removeAttribute('aria-current');}
  };
  return {update,dispose(){lifetime.abort();observers.forEach(observer=>observer.disconnect());frames.forEach(id=>ownerWindow.cancelAnimationFrame(id));timers.forEach(id=>ownerWindow.clearTimeout(id));}};
}
