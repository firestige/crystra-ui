/* eslint-disable @typescript-eslint/ban-ts-comment -- Accepted legacy DOM script; behavior is covered by integration tests. */
// @ts-nocheck -- Preserve accepted v8 script while the host adapter is incrementally typed.
import { taskRowTemplates, workflowRowTemplate } from "./template";
export function mountV8Sidebar(root, callbacks) {
  ...
  const document = new Proxy(ownerDocument, {
    get(target, key) {
  ...
  const window = new Proxy(ownerWindow, {
    get(target, key) {
      if (key === "addEventListener")
        return (type, fn, options) =>
          target.addEventListener(type, fn, {
  ...
  const matchMedia = (query) => {
    ...
    media.addEventListener = (type, fn, options) =>
      add(type, fn, { ...options, signal: lifetime.signal });
    return media;
  };
  const requestAnimationFrame = (fn) => {
    const id = ownerWindow.requestAnimationFrame(fn);
    frames.add(id);
    return id;
  };
  const setTimeout = (fn, ms) => {
    const id = ownerWindow.setTimeout(fn, ms);
    timers.add(id);
    return id;
  };
  const clearTimeout = (id) => ownerWindow.clearTimeout(id);
  const MutationObserver = class extends ownerWindow.MutationObserver {
    constructor(fn) {
      super(fn);
      observers.push(this);
    }
  ...
  const section = (name) =>
    document.querySelector(`[data-section-id="${name}"]`);
  let refreshAttention = () => {};
  ...
  taskViewOptionsAction.addEventListener("click", () => {
    ...
    setTaskViewOptionsOpen(
      taskViewOptionsAction.getAttribute("aria-expanded") !== "true",
    );
  });
  ...
  taskSectionToggle.addEventListener("click", () => {
    setTaskSectionExpanded(
      taskSectionToggle.getAttribute("aria-expanded") !== "true",
    );
  });
  ...
  const renderTaskList = () => {
    const sortMode = document.querySelector(
      'input[name="task-sort"]:checked',
    ).value;
    const filterMode = document.querySelector(
      'input[name="task-filter"]:checked',
    ).value;
    ...
    taskRows
      .sort(
        (left, right) =>
          new Date(right.dataset[timeField]) -
          new Date(left.dataset[timeField]),
      )
      .forEach((row) => taskList.append(row));
    taskRows.forEach((row) => {
      const matchesFilter =
        filterMode === "all" || row.dataset.taskActive === "true";
      const matchesQuery =
  ...
  document
    .querySelectorAll('input[name="task-sort"], input[name="task-filter"]')
    .forEach((input) => input.addEventListener("change", renderTaskList));
  taskSearchInput.addEventListener("input", renderTaskList);
  renderTaskList();

  document.addEventListener("click", (event) => {
    if (
      taskSearchControl.dataset.open === "true" &&
      !taskSearchControl.contains(event.target)
    ) {
      taskSearchInput.value = "";
      setTaskSearchOpen(false);
      renderTaskList();
    ...
    if (
      !taskViewOptionsMenu.hidden &&
      !taskViewOptionsMenu.contains(event.target) &&
      !taskViewOptionsAction.contains(event.target)
    ) {
      setTaskViewOptionsOpen(false);
    }
  });
  ...
  const renderWorkflowList = () => {
    ...
    const direction =
      document.querySelector('input[name="workflow-sort"]:checked').value ===
      "descending"
        ? -1
        : 1;
    ...
    [...workflowList.querySelectorAll("[data-workflow-definition-id]")]
      .sort(
        (a, b) =>
          direction *
          a.dataset.workflowDefinitionId.localeCompare(
            b.dataset.workflowDefinitionId,
          ),
      )
      .forEach((row) => {
        row.hidden = !row.dataset.workflowDefinitionId
          .toLocaleLowerCase()
          .includes(query);
  ...
  workflowSearchClear.addEventListener("click", () => {
    setWorkflowSearchOpen(false);
    workflowSearchAction.focus();
  });
  workflowSearchInput.addEventListener("input", renderWorkflowList);
  workflowViewAction.addEventListener("click", () => {
    setTaskViewOptionsOpen(false);
    setWorkflowViewOpen(workflowViewMenu.hidden);
  });
  workflowToggle.addEventListener("click", () =>
    setWorkflowExpanded(workflowPanel.dataset.expanded !== "true"),
  );
  document
    .querySelectorAll(
      'input[name="workflow-sort"], [data-section-id="workflow-show-version"]',
    )
    .forEach((input) => input.addEventListener("change", renderWorkflowList));
  document.addEventListener(
    "pointerdown",
    (event) => {
      if (!section("workflow-section-header").contains(event.target))
        setWorkflowViewOpen(false);
      if (
        workflowSearchControl.dataset.open === "true" &&
        !workflowSearchControl.contains(event.target)
      )
        setWorkflowSearchOpen(false);
    },
    true,
  ...
  (() => {
    ...
    const routes = {
      tasks: {
        panel: section("task-working-set"),
        toggle: section("task-section-toggle"),
        content: section("task-section-content"),
      },
      workflows: {
        panel: section("workflow-navigation"),
        toggle: section("workflow-section-toggle"),
        content: section("workflow-section-content"),
      },
      analysis: {
        panel: section("analysis-navigation"),
        toggle: section("analysis-section-toggle"),
        content: section("analysis-section-content"),
      },
    };
    let active = null;
    ...
    const syncBanner = () => {
      ...
      section("sidebar-brand-name").textContent = isCrystra
        ? "Crystra"
        : "DeepSeek";
      const label = collapsed()
        ? "展开侧边栏"
        : isCrystra
          ? "切换到 DeepSeek Harness"
          : "切换到 Crystra";
    ...
    const setCollapsed = (value, focusToggle = true) => {
      finishMotion();
      const animate =
        shell.dataset.sidebarMotion === "ready" && !reducedMotion.matches;
      ...
      if (animate) {
        ...
        if (value)
          contentAnimations = [
            section("sidebar-rail").animate([{ opacity: 0 }, { opacity: 1 }], {
              duration: 100,
              delay: 220,
              easing: "ease-out",
              fill: "backwards",
    ...
    document.addEventListener("pointerdown", (event) => {
      if (
        active &&
        !routes[active].panel.contains(event.target) &&
        !event.target.closest('[data-section-id="sidebar-rail"]')
      )
        closePanel(false);
    });
    document.addEventListener(
      ...
      (event) => {
        ...
        if (active) {
          ...
          if (active === "tasks" && !taskViewOptionsMenu.hidden) {
            setTaskViewOptionsOpen(false);
            event.preventDefault();
            return;
          }
    ...
    sidebar.addEventListener("click", (event) => {
      if (
        ...
        event.target.closest(
          'a, [data-task-row], [data-section-id="new-task-action"]',
        )
      )
        closePanel(false);
    });
    const updateAttention = () => {
      const count = [
        ...taskList.querySelectorAll(
          '[data-task-row] [data-section-id$="attention-badge"]',
        ),
      ].reduce(
        (sum, badge) => sum + (Number(badge.textContent.trim()) || 0),
        0,
      );
    ...
    updateAttention();
    new MutationObserver(updateAttention).observe(taskList, {
      subtree: true,
      childList: true,
      characterData: true,
    });
    sidebar
      .querySelectorAll("[data-sidebar-tooltip], [data-header-tooltip]")
      .forEach((button) => {
        const show = () => {
          ...
          tooltipOwner = button;
          tooltip.textContent =
            button.dataset.headerTooltip || button.dataset.sidebarTooltip;
          tooltip.hidden = false;
          const r = button.getBoundingClientRect();
          tooltip.style.left =
            (button.dataset.headerTooltip
              ? Math.max(
                  8,
                  Math.min(
          ...
          tooltip.style.top =
            Math.max(
              8,
              Math.min(
                innerHeight - tooltip.offsetHeight - 8,
                button.dataset.headerTooltip
                  ? r.bottom + 8
                  : r.top + (r.height - tooltip.offsetHeight) / 2,
              ),
            ) + "px";
    ...
    requestAnimationFrame(() =>
      requestAnimationFrame(() => {
        shell.dataset.sidebarMotion = "ready";
      }),
    );
  })();
  const parse = (html) => {
  ...
  const template = parse(taskRowTemplates[1]);
  const workflowTemplate = parse(workflowRowTemplate);
  const bindRows = (items, list, kind) => {
    const old = new Map(
      [...list.children].map((node) => [node.dataset.objectId, node]),
    );
    for (const item of items) {
      let row = old.get(item.id);
      old.delete(item.id);
      if (!row) {
        row = (kind === "task" ? template : workflowTemplate).cloneNode(true);
        row.removeAttribute("data-section-id");
        row.dataset.objectId = item.id;
        row
          .querySelectorAll("[data-section-id]")
          .forEach((node) => node.removeAttribute("data-section-id"));
        row.addEventListener("click", (event) => {
          event.preventDefault();
          callbacks.onNavigate(
            kind,
            row.dataset.objectId,
            row.dataset.revision || undefined,
          );
        });
      }
      ...
      if (kind === "workflow") {
        row.href = "#workflow";
        row.dataset.workflowRevision = item.revision ?? "";
        row.removeAttribute("data-source-task-id");
      }
      row.dataset.taskId = item.id;
      row.dataset.workflowDefinitionId = item.id;
      row.dataset.createdAt = item.createdAt ?? "";
      row.dataset.lastEventAt = item.lastActivityAt ?? "";
      row.dataset.taskActive = item.inactive === true ? "false" : "true";
      const title =
        row.querySelector('[data-type="item-title"]') ??
        (kind === "workflow" ? row.querySelector("span") : null);
      if (title) {
      ...
      if (meta) {
        meta.textContent = kind === "workflow" ? (item.revision ?? "") : "";
        meta.hidden = kind === "task";
      }
      // Unknown activity/status/time/attention must not inherit the template fixture.
      if (kind === "task")
        row
          .querySelectorAll(
            'span:not([data-type="item-title"]):not([data-type="meta"])',
          )
          .forEach((node) => {
            node.hidden = true;
      ...
      const selected =
        callbacks.route === kind && callbacks.selectedId === item.id;
      row.setAttribute("aria-current", String(selected));
      if (kind === "task")
        row.className = parse(taskRowTemplates[selected ? 0 : 1]).className;
      list.append(row);
    }
    for (const row of old.values()) row.remove();
  };
  section("new-task-action").addEventListener("click", () =>
    callbacks.onNewTask(),
  );
  section("host-settings").addEventListener("click", () =>
    callbacks.onOpenSettings(),
  );
  for (const [id, page] of [
    ["all-tasks-action", "tasks"],
    ["all-workflows-action", "workflows"],
    ["analysis-dashboard-action", "analysis-overview"],
    ["analysis-traces-action", "analysis-traces"],
    ["analysis-reports-action", "analysis-reports"],
  ]) {
  ...
  const update = (next) => {
    callbacks = next;
    bindRows(next.tasks, taskList, "task");
    bindRows(next.workflows, workflowList, "workflow");
    renderTaskList();
    renderWorkflowList();
    refreshAttention();
    for (const [id, page] of [
      ...
      if (next.route === page) node.setAttribute("aria-current", "page");
  ...
  return {
    ...
    dispose() {
      ...
      frames.forEach((id) => ownerWindow.cancelAnimationFrame(id));
      timers.forEach((id) => ownerWindow.clearTimeout(id));
    },
  };
}
