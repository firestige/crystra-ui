...
const image = "crystra-ui-panel-benchmark-v1:local";
...
const provider =
  process.env.CRYSTRA_BENCHMARK_PROVIDER_COMMIT ?? "working-tree";
const run = await execFile(
  ...
  [
    ...
    "4",
    "--env",
    `CRYSTRA_BENCHMARK_PROVIDER_COMMIT=${provider}`,
    "--env",
    `CRYSTRA_BENCHMARK_RUNNER_IMAGE=${inspected.stdout.trim()}`,
    "--volume",
